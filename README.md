...
| `OPENLIST_RATE_LIMIT_QPS` | `0` | OpenList API 令牌桶 QPS；`0` 表示关闭额外限速 |
| `OPENLIST_RATE_LIMIT_BURST` | `1` | OpenList API 令牌桶突发值；保守模式建议保持 `1` |
...
| `STRM_SYNC_PROFILE` | none | 一键应用同步档位预设，支持 `conservative` / `balanced` / `aggressive`；单项 `STRM_*` 和 `OPENLIST_RATE_LIMIT_*` 变量仍可覆盖对应值 |
