...
4. 如果启用 `OPENLIST_DIRECT_PLAY=true`，`DirectStreamUrl` 也会改到同一条短效链路。
...
| --- | --- | --- |
| `OPENLIST_BASE_URL` | none | OpenList 地址 |
...
| `OPENLIST_DIRECT_PLAY` | `true` | 是否改写 `DirectStreamUrl`，让客户端直接走短效播放入口 |
